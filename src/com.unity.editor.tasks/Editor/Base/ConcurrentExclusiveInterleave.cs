	...
	public sealed class ConcurrentExclusiveInterleave
	{
				...
				finally
				{
					if (cleanupOnExit)
					{
						lock (internalLock)
						{
							...
							if (((Queue<Task>)concurrentTaskScheduler.Tasks).Count == 0 && ((Queue<Task>)exclusiveTaskScheduler.Tasks).Count == 0)
							{
								taskExecuting = null;
								runTasks = false;
							}
		...
		private IEnumerable<Task> GetConcurrentTasksUntilExclusiveExists()
		{
			while (true)
			{
				...
				lock (internalLock)
				{
					if (((Queue<Task>)exclusiveTaskScheduler.Tasks).Count == 0 && ((Queue<Task>)concurrentTaskScheduler.Tasks).Count > 0)
					{
						foundTask = ((Queue<Task>)concurrentTaskScheduler.Tasks).Dequeue();
					}
				}
				if (token.IsCancellationRequested) yield break;
		...
		private IEnumerable<Task> GetExclusiveTasks()
		{
			while (true)
			{
				...
				lock (internalLock)
				{
					if (((Queue<Task>)exclusiveTaskScheduler.Tasks).Count > 0) foundTask = ((Queue<Task>)exclusiveTaskScheduler.Tasks).Dequeue();
				}
				if (token.IsCancellationRequested) yield break;
				if (foundTask != null) yield return foundTask;
				else yield break;
		...
		private int ExclusiveTaskCount
		{
			get
			{
				lock (internalLock) return ((Queue<Task>)exclusiveTaskScheduler.Tasks).Count;
			}
		}
		...
		private int ConcurrentTaskCount
		{
			get
			{
				lock (internalLock) return ((Queue<Task>)concurrentTaskScheduler.Tasks).Count;
			}
		}
